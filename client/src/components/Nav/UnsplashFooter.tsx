...
export const UnsplashFooter = () => {
  ...
  const fetchRandomPhoto = async () => {
    const response = await fetch(`https://api.unsplash.com/photos/random?client_id=${ACCESS_KEY}&orientation=landscape&collections=11649432`)
  ...
  return (
    <div className="flex fixed bottom-0 right-0 space-x-4">
      <a href={dailyUnsplash.links?.html} target="_blank" className="flex items-center text-white text-sm" >Image Source</a>
        <button

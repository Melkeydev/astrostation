...
export const HackerNews = () => {
  ...
  const { isLoading, error, data, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`https://hacker-news.firebaseio.com/v0/${selectedStoryType.apiSrc}.json`).then();
      ...
      const data = await Promise.all(
        ids.map(async id => axios.get(`https://hacker-news.firebaseio.com/v0/item/${id}.json`))
      );
  ...
  if (isLoading) {
    return (
      <LoaderCard
        title="Hacker News"
        reloadFunction={refetchStories}
        toggleFunction={setIsHackerNewsToggled}
        refreshing={true}
        width={768}
        height={490}
      />
  ...
  return (
    <Card
      title={`Hacker News - ${selectedStoryType.title}`}
      toggleFunction={setIsHackerNewsToggled}
      reloadFunction={refetchStories}
      width="sm:w-[48rem]"
    >
        ...
        {data.map(story => {
          return (
            <li key={story.data.id} className="py-1">
              <a className="font-semibold" href={story.data.url} target="_blank" rel="noferrer noopener">
                {story.data.title}
              </a>
            </li>
          );
        ...
        {storyTypes.map(storyType => {
          return (
            <li key={storyType.key}>
              <button
                onClick={() => updateNewsFeed(storyType.key)}
                className="bg-white/[.80] dark:border-gray-700 dark:bg-gray-700/[.96] dark:text-gray-300 py-1 px-3 ml-[2px]"
              >
                <span className={clsx(selectedStoryType.key === storyType.key && "underline")}>{storyType.name}</span>
              </button>
            </li>
          );
        })}
